...
**离散化 (Discretization)**：为了在计算机上处理离散的序列数据（如文本），我们需要将连续系统离散化。常用的方法是**零阶保持 (Zero-Order Hold, ZOH)**。
离散化后，公式变为递归形式：
...
### 半可分离矩阵 (Semiseparable Matrix)

接下来我们对Mamba架构进行进一步推导：

对于整个序列 $x = [x_1, \dots, x_L]$ 和输出 $y = [y_1, \dots, y_L]$，把Mamba的公式展开，我们会发现这个变换可以写成 $y = M x$，其中 $M$ 是一个下三角矩阵。
